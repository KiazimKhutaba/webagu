        ...
        <div class="mx-1">
            <q-table :cols="cols" :rows="quizzes">
                <template #item="{item}">
                    ...
                    <td>{{ department_translate(item?.student_department) }}</td>
                    <td>
                    ...
                    <td>
                        <router-link :to="url('quizzes.passed', {id: item.id})">Посмотреть</router-link>
                    </td>
                </template>
            </q-table>
        </div>
...
export default {
    ...
    async mounted() {
        const { data } = await quizService().getPassedQuizzes(this.$route.query);
        //console.log(data);
