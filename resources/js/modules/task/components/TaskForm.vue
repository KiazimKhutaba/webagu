<template>
    ...
    <form method="POST" enctype="multipart/form-data" @submit.prevent="handleSubmit" class="mb-3">
        ...
        <div class="mb-3">
            <label for="content" class="form-label fw-bold">Описание</label>
            <div class="form-control min-h-200px" contenteditable="true" ref="description"></div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async handleSubmit() {
            ...
            try {
                const {data} = await tasksService().create(this.getForm());
